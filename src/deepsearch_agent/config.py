    ...
    return Settings(
        llm=LLMConfig(
            ...
            context_window_tokens=max(
                4_096, _int_env("LLM_CONTEXT_WINDOW_TOKENS", 32_768)
            ),
        ...
        agent=AgentConfig(
            ...
            max_post_review_recovery_cycles=max(
                0,
                _int_env("AGENT_MAX_POST_REVIEW_RECOVERY_CYCLES", 1),
            ),
            ...
            partial_report_min_sources=max(
                1, _int_env("AGENT_PARTIAL_REPORT_MIN_SOURCES", 1)
            ),
            writer_read_batch_size=max(1, _int_env("AGENT_WRITER_READ_BATCH_SIZE", 5)),
            ...
            report_max_topics=max(
                1, _int_env("AGENT_REPORT_MAX_TOPICS", 6)
            ),
            report_max_caveats=max(
                1, _int_env("AGENT_REPORT_MAX_CAVEATS", 6)
            ),
            ...
            research_agent_max_evidences_per_direction=max(
                1,
                _int_env("AGENT_RESEARCH_MAX_EVIDENCES_PER_DIRECTION", 6),
            ),
            ...
            evidence_extract_timeout=max(
                1.0, _float_env("AGENT_EVIDENCE_EXTRACT_TIMEOUT", 120.0)
            ),
            ...
            research_failure_history_limit=max(
                1, _int_env("AGENT_RESEARCH_FAILURE_HISTORY_LIMIT", 5)
            ),
        ),
        search=SearchConfig(
            provider=cast(
            ...
            tavily_include_raw_content=_env("TAVILY_INCLUDE_RAW_CONTENT", "true").lower()
            not in {"0", "false", "no"},
        ),
        app=AppConfig(
            environment=_env("APP_ENV", "development"), log_level=_env("LOG_LEVEL", "INFO").upper()
        ),
        observability=ObservabilityConfig(
