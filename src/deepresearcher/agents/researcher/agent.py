...
class ResearchAgent:
    ...
    async def run(self, task: SubTask) -> ResearchAgentResult:
        ...
        loop_context = self._build_loop_context(task, loop_state)
    ...
    def _build_loop_context(
        ...
        async def search_sources(queries: list[str], reason: str) -> dict[str, object]:
            return await self._search_sources(
                task, queries, reason, loop_state=loop_state, event_context=event_context
            )
        ...
        *,
        ...
        async with commit_lock:
            ...
            per_source = {
                ...
                for source_url in {
                    document.source_url for document in loop_state.documents.values()
                }
            }
    ...
    async def _search_sources(
        ...
        *,
        ...
        del reason
        remaining = self.config.research_agent_max_queries - len(loop_state.queries)
        new_queries = self._new_queries(proposed_queries, loop_state.queries)[: max(0, remaining)]
        if not new_queries:
    ...
    async def _list_search_results(
        ...
        *,
        ...
        cards, next_offset = self._candidate_cards(
            loop_state, raw_results, start=offset, count=limit
        )
        return {
            "status": "completed",
            "search_id": search_id,
